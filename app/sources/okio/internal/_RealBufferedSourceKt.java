...
public final class _RealBufferedSourceKt {
    ...
    public static final boolean commonExhausted(RealBufferedSource $this$commonExhausted) {
        ...
        if (!(!$this$commonExhausted.closed)) {
            throw new IllegalStateException("closed".toString());
        } else if (!$this$commonExhausted.bufferField.exhausted() || $this$commonExhausted.source.read($this$commonExhausted.bufferField, 8192) != -1) {
            return false;
        } else {
            return true;
        }
    ...
    public static final String commonReadUtf8LineStrict(RealBufferedSource $this$commonReadUtf8LineStrict, long limit) {
        ...
        if (j >= 0) {
            ...
            if (scanLength < Long.MAX_VALUE && realBufferedSource.request(scanLength) && $this$commonReadUtf8LineStrict.bufferField.getByte(scanLength - 1) == ((byte) 13) && realBufferedSource.request(1 + scanLength) && $this$commonReadUtf8LineStrict.bufferField.getByte(scanLength) == b2) {
    ...
    public static final long commonIndexOfElement(RealBufferedSource $this$commonIndexOfElement, ByteString targetBytes, long fromIndex) {
        ...
        if (!realBufferedSource.closed) {
            while (true) {
                ...
                if (realBufferedSource.source.read($this$commonIndexOfElement.bufferField, 8192) == -1) {
                    return -1;
                }
                fromIndex2 = Math.max(fromIndex2, lastBufferSize);
            }
